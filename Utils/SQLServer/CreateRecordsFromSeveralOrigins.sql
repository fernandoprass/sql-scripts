...
SET @cont = 0
SET @validarLancamentoTrf = 'S' --por default valida os saldo dos lan�amento da TRF
...
IF(@nomeTabela = 'TRF')
BEGIN
   --se for lancamento para o mesmo projeto da TRF, n�o valida pois "provavelmente" seja uma TRF para ajuste de saldo das metas
...
--verifica se existe o registro na tabela LANCAMENTO_MES fechado (n�o permitindo lan�amentos)
...
IF(@dataHoraLancamentoTabela IS NOT NULL AND @nomeTabela <> 'ADIANTAMENTO' AND @nomeTabela <> 'LANCAMENTO_ESTORNO')
   SET @retorno = 3 --J� foram gerados os lan�amentos para este item
ELSE IF(@idTciOrigemPagamento IS NULL)
   SET @retorno = 6 --O campo Origem do Pagamento � de preenchimento obrigat�rio
ELSE IF(@idTciFormaPagamento IS NULL)
   SET @retorno = 7 --O campo Forma de Pagamento � de preenchimento obrigat�rio
ELSE IF((SELECT ISNULL(PERMITE_LANCAR,'N') FROM USUARIO WHERE ID_USUARIO = @idUsuario) = 'N')
   SET @retorno = 40 --Voc� n�o tem permiss�o para realizar essa opera��o. Consulte o Administrador do Sistema.
...
BEGIN TRY
   ...
   IF(@idProjetoMeta IS NULL)
      SET @idProjetoMeta = -1 --seta como -1 pois o usuario pode n�o ter selecionado a meta para fazer o rateio
END TRY
BEGIN CATCH
...
BEGIN
   BEGIN TRY
      --se o nome for diferente do padr�o da meta
      ...
      BEGIN
         BEGIN TRY
            ...
            --busca o nome do ID que ser� usado na busca da meta
            ...
            --busca a meta na tabela que faz referencia a que est� sendo inserida
            SELECT @sqlMeta = 'SELECT @projetoMeta = ' + ISNULL(@nomeCampoProjetoMeta,'ID_PROJETO_META') + ' '
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               SET @retorno = 150 --A Data do Documento n�o pode ser maior que a Data do Lan�amento
         ...
         --se for para n�o vincular o projeto ao lan�amento, seta como nulo
         ...
         BEGIN
            ...
            BEGIN
               ROLLBACK TRANSACTION
               SET @retorno = 25 --� obrigat�rio o preenchimento da Conta Cont�bil
               CLOSE crLancamentoContabil
               DEALLOCATE crLancamentoContabil
               RETURN
            END
         ...
         BEGIN
            ...
            --SE N�O ACHOU A CC DA META, BUSCA DO PROJETO
            IF(@lIdContaContabil IS NULL)
            BEGIN
               IF (@lIdProjeto IS NULL) --se o projeto eh nulo busca a Conta da Funda��o
               BEGIN
                  SELECT @lIdContaContabil=ID_CONTA_CONTABIL
                  FROM dbo.FUNDACAO
               ...
               BEGIN
                  ...
                  BEGIN
                    ROLLBACK TRANSACTION
                       SET @retorno = 8 --N�o foi poss�vel gerar os lan�amentos. Projeto sem Conta Cont�bil cadastrada
                    CLOSE crLancamentoContabil
                    DEALLOCATE crLancamentoContabil
                    RETURN             
                  END
         ...
         BEGIN
            ...
            --se n�o for um historico padr�o, busca na tabela de configura��o a origem
            ...
            BEGIN
               ROLLBACK TRANSACTION
               SET @retorno = 121 --Projeto do tipo SICONV n�o permite essa Forma de Pagamento
               CLOSE crLancamentoContabil
               DEALLOCATE crLancamentoContabil
               RETURN
            END
            ...
            --excess�o a regra, esta conta 3 deve ter um projeto relacionado
            ...
            BEGIN
               ROLLBACK TRANSACTION
               SET @retorno = 46 --N�o � permitido gerar Lan�amentos para a conta 311.102 sem Projeto relacionado
               CLOSE crLancamentoContabil
               DEALLOCATE crLancamentoContabil
               RETURN
            END        
            ...
            BEGIN
               ...
               BEGIN
                  ROLLBACK TRANSACTION
                  SET @retorno = 39 --N�o � permitido gerar Lan�amentos para Pessoa sem CPF/CNPJ
                  CLOSE crLancamentoContabil
                  DEALLOCATE crLancamentoContabil
                  RETURN
               END
               ...
               BEGIN
                  ROLLBACK TRANSACTION
                  SET @retorno = 52 --A Pessoa informada n�o possui PIS/PASEP cadastrado. N�o � poss�vel gerar os lan�amentos.
                  CLOSE crLancamentoContabil
                  DEALLOCATE crLancamentoContabil
                  RETURN
               END             
               ...
               BEGIN
                  ROLLBACK TRANSACTION
                  SET @retorno = 45 --A Pessoa informada � funcion�rio da Funda��o. N�o � poss�vel gerar os lan�amentos.
                  CLOSE crLancamentoContabil
                  DEALLOCATE crLancamentoContabil
                  RETURN
               END               
            ...
            --altera a string do complemento com os dados do lan�amento
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  BEGIN
                     ROLLBACK TRANSACTION
                     SET @retorno = 55 --'N�o � permitido gerar Lan�amentos para esta rubrica pois deve ser registrada na Central Despesa.'
                     CLOSE crLancamentoContabil
                     DEALLOCATE crLancamentoContabil
                     RETURN
                  END
            ...
            END
            ELSE --gera o registro em Lan�amento
            BEGIN
               ...
               BEGIN
                  ...
                  BEGIN
                     ROLLBACK TRANSACTION
                     SET @retorno = 164 --Esta Pessoa n�o est� liberada para receber Pagamentos.'
                     CLOSE crLancamentoContabil
                     DEALLOCATE crLancamentoContabil
                     RETURN
                  END               
               ...
               BEGIN
                  ...
                  --se um dos dados banc�rios n�o existir, n�o deixar gerar lan�amento para pagamento por border�
                  IF(@retorno > 0)
                  BEGIN
                     ROLLBACK TRANSACTION
                     CLOSE crLancamentoContabil
   ...
   BEGIN CATCH
      ROLLBACK TRANSACTION
      SET @retorno = 5 --Erro ao Gerar os Lan�amentos
      CLOSE crLancamentoContabil
      DEALLOCATE crLancamentoContabil
      RETURN
   END CATCH
...
BEGIN
   ROLLBACK TRANSACTION
   SET @retorno = 9 --N�o existem par�metros para gera��o dos lan�amentos
   CLOSE crLancamentoContabil
   DEALLOCATE crLancamentoContabil
   RETURN
END
...
BEGIN
   ROLLBACK TRANSACTION
   SET @retorno = 1 --Erro n�o identificado
   RETURN
END